...
export async function retrieveTranscripts(
  ...
  // If the router returned no terms, ground the debate in the most
  // recent transcripts so advocates at least have something to cite.
  if (!cleaned.length) return recentTranscripts(limit);

  // ["slack integration", "notifications"] -> "(slack & integration) | (notifications)"
  const query = cleaned
  ...
  const { data, error } = await supabase
    .from('transcripts')
    .select(`
      id, lead_id, created_at, raw_text, ai_summary, ai_sentiment,
      ai_interest_level, ai_next_steps, ai_pain_points, ai_product_feedback,
      ai_key_quotes, ai_follow_up_suggestions,
