...
export async function draftFollowUp(lead: Lead, lastInteractionSummary: string, daysSince: number): Promise<string> {
  const systemPrompt = `You're drafting a follow-up email for a startup founder. Keep it casual, brief (2-3 sentences), and human. No corporate speak. Be direct and unpretentious. Return just the email body, no subject line.`;
