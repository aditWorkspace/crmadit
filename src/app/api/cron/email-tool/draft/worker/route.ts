...
// worker_locked_until, same pattern as the enrich worker), researches +
// writes each via processDraftRow, and persists the outcome. Bounded by a
// time budget, a per-run count, and a rolling 24h spend ceiling.
//
...
export async function POST(req: NextRequest) {
  ...
  while (Date.now() - startMs < DRAFT_WORKER_BUDGET_MS && stats.processed < DRAFT_WORKER_BATCH) {
    ...
    if (spent >= ceiling) {
      return NextResponse.json({ ok: true, note: 'spend_ceiling_reached', spent_24h: spent, ceiling, ...stats });
    }
    ...
    if (outcome.kind === 'ready') {
      await supabase.from('cold_email_drafts').update({
        ...
        body: outcome.body,
        opener_tier: outcome.opener_tier,
        signal_score: outcome.signal_score,
        evidence_cards: outcome.evidence_cards,
        selected_evidence_ids: outcome.selected_evidence_ids,
        research_model: 'sonar',
        decider_model: COLD_RESEARCH_MODEL,
        writer_model: COLD_WRITER_MODEL,
        written_at: nowIso,
        ready_at: nowIso,
        error: null,
      }).eq('id', draft.id);
      ...
      await supabase.from('cold_email_drafts').update({
        ...base, status: 'skipped', skip_reason: outcome.reason,
      }).eq('id', draft.id);
      stats.skipped++;
      ...
      if (attempt < MAX_DRAFT_ATTEMPTS) {
        ...
        await supabase.from('cold_email_drafts').update({
          ...
          retry_at: new Date(Date.now() + backoff).toISOString(),
      ...
      await supabase.from('cold_email_drafts').update({
        ...
        status: 'failed',
        error: outcome.reason,
        evidence_cards: outcome.evidence_cards ?? null,
      }).eq('id', draft.id);
      stats.failed++;
    }
